...
(struct user-manager (db hasher) #:transparent)
                  ...
                  [(user-manager-lookup/username um username)
                   ...
                   (lambda (u)
                     (~> (from password-reset #:as pr)
                         ...
                         (query-exec conn _))
                     (values u
                             (~> (make-password-reset #:user-id (user-id u)
                                                      #:ip-address ip-address
                                                      #:user-agent user-agent)
                                 (insert-one! conn _)
...
(define (user-manager-lookup/id um id)
  (with-timing 'user-manager
               (format "(user-manager-lookup/id ~v)" id)
               (with-database-connection
                [conn (user-manager-db um)]
                (~> (from user #:as u) (where (= u.id ,id)) (lookup conn _)))))
                  ...
                  [(lookup-password-reset conn id token)
                   ...
                   (lambda (_pr)
                     (begin0 #t
                       ...
                       (and~> (lookup conn (~> (from user #:as u) (where (= u.id ,id))))
...
(define (lookup-password-reset conn id token)
  (~> (from password-reset #:as pr)
      (where (and (= pr.user-id ,id) (= pr.token ,token) (> pr.expires-at (now))))
      (lookup conn _)))
