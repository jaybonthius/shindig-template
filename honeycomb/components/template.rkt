...
(define (nav . items)
  (haml (.nav ([:up-nav ""]) (container (haml (.nav__items ,@items))))))

(define (nav-item uri label)
  (haml (:li.nav__item (:a ([:href uri] [:up-alias uri]) label))))

(define (page #:subtitle [subtitle #f] #:show-nav? [show-nav? #t] . content)
     ...
     (haml (:html (:head (:meta ([:charset "utf-8"]))
                  ...
                  (:body (when show-nav?
                         ...
                         (unless (null? (current-flash-messages))
                           (container
                            (haml (:ul.flash ,@(for/list ([flash (current-flash-messages)])
                                                 (haml (:li ([:class
                                                              (format "flash__item flash__item--~a"
                                                                      (car flash))])
                                                            (cdr flash))))))))
                         (.content ,@content)))))
   (response 200
