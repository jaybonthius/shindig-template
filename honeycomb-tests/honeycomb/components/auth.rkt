...
(define user #f)
(define auth-tests
  (system-test-suite auth
                     ([auth (sessions users) make-auth-manager] [db make-test-database]
                     ...
                     (test-suite "auth-manager-login"

                       (test-case "returns #f if the user does not exist"
                         (check-false (auth-manager-login! auth "idontexist" "hunter2")))

                       (test-case "returns #f if the password is wrong"
                         (check-false (auth-manager-login! auth "bogdan" "invalid")))

                       (test-case "fails if the user is not verified"
                         (check-exn exn:fail:auth-manager:unverified?
                                    (lambda () (auth-manager-login! auth "bogdan" "hunter2"))))
