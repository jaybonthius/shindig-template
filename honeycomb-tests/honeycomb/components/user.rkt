...
(define user-tests
  (system-test-suite
   user
   ([db make-test-database] [hasher make-test-hasher]
                            [users (db hasher) make-user-manager]
                            [migrator (db) make-test-migrator])
   #:before (lambda ()
              (truncate-tables! db 'users)
              (set! the-user (make-test-user! users)))
   (test-suite "user-manager-create!"

     (test-case "creates users"
       (check-match the-user
                    (struct* user
                             ([id (? exact-positive-integer?)] [username "bogdan@example.com"]
                                                               [verified? #f])))))
   (test-suite "user-manager-create-reset-token!"
     ...
     (test-case "returns a token given a valid user"
       ...
       (test-case "invalidates old tokens"
         ...
         (check-false (user-manager-reset-password! users
                                                    #:user-id (user-id the-user)
                                                    #:token token
                                                    #:password "new-password"))

         (check-true (user-manager-reset-password! users
                                                   #:user-id (user-id the-user)
                                                   #:token token*
                                                   #:password "hunter2"))
   ...
   (test-suite "user-manager-login"
     ...
     (test-case "returns a user upon successful login"
       (check-match (user-manager-login users "bogdan@example.com" "hunter2")
                    (struct* user
                             ([id (? exact-positive-integer?)] [username "bogdan@example.com"])))))))
