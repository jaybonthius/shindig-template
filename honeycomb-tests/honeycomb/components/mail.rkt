...
(define mailer (make-test-mailer))
...
(define mail-tests
  (system-test-suite
   ...
   ([db make-test-database] [hasher make-test-hasher]
                            ...
                            [users (db hasher mailer) (lambda (db h _) (make-user-manager db h))])
   #:before (lambda () (truncate-tables! db 'users))
   (test-suite "mailer-send-welcome-email"
     ...
     (test-case "welcome emails contain valid verification urls"
       ...
       (check-equal? (~> (car (stub-mail-adapter-outbox adapter))
