...
(provide make-test-database
         make-test-migrator
         run-db-tests)

(define-runtime-path migrations-path (build-path 'up 'up "migrations"))

(define (make-test-database-connection)
  (postgresql-connect #:server config:test-db-host
...
(define make-test-migrator (make-migrator-factory migrations-path))
...
(provide make-test-hasher)

(define (make-test-hasher)
  ((make-argon2id-hasher-factory #:parallelism 1 #:iterations 128 #:memory 128)))

;; mail ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(provide make-test-mailer)
...
(provide make-test-session-manager)
...
(provide make-test-user!)

(define (make-test-user! users [username "bogdan@example.com"] [password "hunter2"])
