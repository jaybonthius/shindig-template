...
(provide (all-from-out koyo/mail)
         (contract-out [mailer-send-welcome-email (-> mailer? user? void?)]
...
(define (mailer-send-welcome-email m user)
  ...
  (mail-adapter-send-email-with-template (mailer-adapter m)
                                         #:to (user-username user)
                                         #:from (mailer-sender m)
                                         #:template-alias "welcome"
                                         #:template-model
                                         (mailer-merge-common-variables m
                                                                        'action_url
                                                                        action-url
                                                                        'name
                                                                        (user-username user)
...
(define (mailer-send-password-reset-email m user token)
  (define action-url (make-application-url "password-reset" (number->string (user-id user)) token))
