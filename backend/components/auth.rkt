...
(provide exn:fail:auth-manager?
         exn:fail:auth-manager:unverified?

         (contract-out [current-user (parameter/c (or/c #f user?))]
...
(struct auth-manager (sessions users) #:transparent)
    ...
    [(user-manager-login (auth-manager-users am) username password)
     ...
     (lambda (u)
       (unless (user-verified? u)
         (raise (exn:fail:auth-manager:unverified "this user is not verified"
               ...
               (cond
                 ...
                 [else
                  (redirect-to (reverse-uri 'login-page
                                            #:query
                                            `((return . ,(url->string (request-uri req))))))])))
